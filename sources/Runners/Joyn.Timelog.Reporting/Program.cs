...
builder.ConfigureServices((hostContext, services) =>
{
    ...
    services.AddHostedService<Joyn.Timelog.Reporting.LogServerClient>();
    services.AddHostedService<Joyn.Timelog.Reporting.ViewerServer>();
});
...
Joyn.Timelog.Reporting.LogServerClient.Startup(applicationKey, logServerClientConfiguration, logger);

var viewerServerConfiguration = configuration.GetSection("ViewerServer").Get<ViewerServerConfiguration>();
Joyn.Timelog.Reporting.ViewerServer.Startup(viewerServerConfiguration, logger);
...
do
{
    ...
    if (readConsole == "1")
    {
        ...
        foreach (var client in Joyn.Timelog.Reporting.ViewerServer.ListClients())
        {
    ...
    else if (readConsole == "2")
    {
        ...
        foreach(var filter in Joyn.Timelog.Reporting.ViewerFiltersHandler.ListFilters())
        {
        ...
        Joyn.Timelog.Reporting.ViewerFiltersHandler.ListFilters();
    }
    else if (readConsole != "")
    {
        Joyn.Timelog.Reporting.ViewerServer.BroadcastMessage(readConsole);
    }
